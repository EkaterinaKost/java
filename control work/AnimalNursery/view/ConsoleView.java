...
import AnimalNursery.util.Counter;
...
public class ConsoleView implements View {
    ...
    public ConsoleView(Accounting Accounting) {
    ...
    public void showKennelRegistry() {
        ...
        List<AbstractAnimal> animals = Accounting.getAnimals();
    ...
    public boolean showAddAnimalDialog() {
        ...
        while (true) {
            ...
            try (counter){
                ...
                counter.add();
                return Accounting.createAnimal(parsedData.name, parsedData.birthDate, parsedData.animalGenius);
    ...
    public void showDetailInfoAnimalDialog() {
        String infoMessage = String.format("Введите номер животного (1 - %d)", Accounting.getAnimals().size());
        System.out.println(infoMessage);
        while (true) {
            try {
                scanner = new Scanner(System.in);
                int animalNumber = scanner.nextInt();
                AbstractAnimal animal = Accounting.getAnimals().get(animalNumber - 1);
    ...
    public int showRemoveAnimalDialog() {
        ...
        while (true) {
            try {
                ...
                return Accounting.removeAnimal(animal);
            } catch (IndexOutOfBoundsException e) {
                System.out.println("Запись с таким номером отсутствует");
            }
        }
